...
def drawFirst(draw):
    ...
    i = 1
    while i <= 6:
        x0 = random.randint(16, 645)
        y0 = random.randint(16, 645)
        startCord = (x0, y0)
        x1 = x0 + random.randint(210, 420) # X length
        y1 = y0 + random.randint(210, 420) # Y length
        endCord = (x1, y1)
        draw.rectangle((startCord, endCord), fill='hsl(240, 0%, 25%)') # (black=0%, normal=50%, white=100%)
        i += 1
